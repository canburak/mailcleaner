...
export const accountsApi = {
  ...
  test: (id: number) => api.post<ConnectionStatus>(`/accounts/${id}/test`).then(r => r.data),

  testDirect: (data: AccountCreate) =>
    api.post<ConnectionStatus>('/accounts/test', data).then(r => r.data),

  getFolders: (id: number) => api.get<Folder[]>(`/accounts/${id}/folders`).then(r => r.data),
...
export const rulesApi = {
  list: (accountId: number) => api.get<Rule[]>(`/accounts/${accountId}/rules`).then(r => r.data),
...
export const previewApi = {
  preview: (accountId: number, folder = 'INBOX', limit = 100) =>
    api
      .get<PreviewResult>(`/accounts/${accountId}/preview`, {
        params: { folder, limit },
      })
      .then(r => r.data),
  ...
  apply: (accountId: number, folder = 'INBOX', dryRun = false) =>
    api
      .post<PreviewResult>(`/accounts/${accountId}/apply`, null, {
